...
export default class Wishlist extends LightningElement {
    ...
    @track currentPage = 1;  // Track the current page
    itemsPerPage = 6;  // 6 records per page (3 rows * 2 columns)
    wiredWishlistItemsResult; // Store the wired result for refresh
    ...
    @wire(getWishlistItems)
    wiredWishlistItems(result) {
        this.wiredWishlistItemsResult = result; // Capture the wire response for refresh
        const { data, error } = result;
        if (data) {
            this.wishlistItems = data;
        } else if (error) {
    ...
    handleRemove(event) {
        ...
        removeFromWishlist({ wishlistItemId: itemId })
            .then(result => {
                this.showToast('Success', result, 'success');
                return refreshApex(this.wiredWishlistItemsResult); // Refresh the wishlist items
            })
            .catch(error => {
                this.showToast('Error', error.body.message, 'error');
            });
